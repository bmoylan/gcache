...
func TestLoaderFunc(t *testing.T) {
	size := 2
	var testCaches = []*CacheBuilder[int64, int64, int64]{
		New[int64, int64](size).Simple(),
		New[int64, int64](size).LRU(),
		New[int64, int64](size).LFU(),
		New[int64, int64](size).ARC(),
	}
...
func TestLoaderExpireFuncWithoutExpire(t *testing.T) {
	...
	for _, builder := range testCaches {
		...
		counter := 1000
		cache := builder.
			LoaderExpireFunc(func(key int64) (int64, *time.Duration, error) {
...
func TestLoaderPurgeVisitorFunc(t *testing.T) {
	...
	tests := []struct {
		name         string
		cacheBuilder *CacheBuilder[int64, int64, int64]
	}{
		{
			name:         "simple",
...
func TestDeserializeFunc(t *testing.T) {
	...
	for _, cs := range cases {
		...
		cc := NewWithSerializedType[string, string, []byte](32).
			EvictType(cs.tp).
			LoaderFunc(func(k string) (string, error) {
				return value1, nil
			}).
			DeserializeFunc(func(k string, v []byte) (string, error) {
			...
			SerializeFunc(func(k string, v string) ([]byte, error) {
