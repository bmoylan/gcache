...
func TestLFUFreqListOrder(t *testing.T) {
	...
	if l := gc.(*LFUCache[int, int, int]).freqList.Len(); l != 6 {
		t.Fatalf("%v != 6", l)
	}
	var i uint
	for e := gc.(*LFUCache[int, int, int]).freqList.Front(); e != nil; e = e.Next() {
	...
	if l := gc.(*LFUCache[int, int, int]).freqList.Len(); l != 5 {
		t.Fatalf("%v != 5", l)
	}
	gc.Set(1, 1)
	if l := gc.(*LFUCache[int, int, int]).freqList.Len(); l != 5 {
...
func TestLFUFreqListLength(t *testing.T) {
...

	{
		...
		if l := gc.(*LFUCache[string, string, string]).freqList.Len(); l != 1 {
			t.Fatalf("%v != 1", l)
		}
	}
	{
		...
		if l := gc.(*LFUCache[string, string, string]).freqList.Len(); l != 2 {
			t.Fatalf("%v != 2", l)
		}
	}
